# python -m streamlit.cli run "C:\Users\aadit\PycharmProjects\HeartDisease\WebApp.py"
...
image = Image.open(
    'C:/Users/aadit/PycharmProjects/HeartDisease/heart disease prediction.png')
st.image(image, caption='ML', use_column_width=True)

#import data
heart_data = pd.read_csv(
    'C:/Users/aadit/PycharmProjects/HeartDisease/heart.csv')
...
def get_user_input():
    ...
    sex = st.sidebar.selectbox('Gender', ('1 - Male', '0 - Female'))
    cp = st.sidebar.selectbox('Does patient have chest pain?', ('1 - typical angina',
                              '2 - atypical angina', '3 - non-anginal pain', '4 - asymptomatic'))
    trestbps = st.sidebar.slider(
        'Patient resting blood pressure (in mm Hg on admission to the hospital)', 50, 250, 50)
    chol = st.sidebar.slider(
        'Patient serum cholestoral in mg/dl', 100, 600, 150)
    fbs = st.sidebar.slider(
        'Patient fasting blood sugar > 120 mg/dl? Select 0 for False and 1 for True', 0, 1, 1)
    restecg = st.sidebar.selectbox('Resting electrocardiographic results', ('0 - Normal', '1 - having ST-T wave abnormality (T wave inversions and/or ST elevation or depression of > 0.05 mV)',
                                   'showing probable or definite left ventricular hypertrophy by Estes criteria'))
    thalach = st.sidebar.slider(
        'Patient maximum heart rate achieved', 50, 250, 180)
    exang = st.sidebar.selectbox(
        'Does patient have exercise induced angina?', ('1 - Yes', '0 - No'))
    ...
    thal = st.sidebar.selectbox('Patient history with thalassemia',
...
features_train_prediction = el.predict(features_test)
...
if prediction == 0:
    st.subheader('The model has predicted that you do not have heart disease')
else:
